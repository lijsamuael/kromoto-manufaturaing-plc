...
export default function Admin(){
    ...
    const updateProduct = async (id, product) => {
        const toBeUpdatedProduct = doc((db, "product", id))
        const newValue = {name:name, description: description, image: selectedImage}
        await updateDoc((toBeUpdatedProduct, newValue))
    }

    const deleteProduct = async (id) => {
        const toBeDeletedProduct = doc((db, "product", id));
        await deleteDoc(toBeDeletedProduct);
    }
            ...
            <div>
                ...
                <div role="alert" class="container mx-auto w-11/12 md:w-2/3 max-w-lg">
                    <div class="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
                        ...
                        <input onChange={(e) => setUpdatedName(e.target.value)} id="name" class="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" placeholder="Enter product name" />

                        <label for="expiry" class="text-gray-800 text-sm font-bold leading-tight tracking-normal">Enter Description</label>
                        <textarea onChange={(e) => setUpdatedDescription(e.target.value)}  name="" className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-20 flex items-center pl-3 text-sm border-gray-300 rounded border"  />
                        ...
                        <div class="flex items-center justify-between w-full pt-4">
                            ...
                            <button onClick={updateProduct(doctor.id, {name: updatedName, description: updatedDescription, image: updatedSelectedImage})} class="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 transition duration-150 ease-in-out hover:bg-secondary-dark2 bg-secondary-dark1 rounded text-white px-8 py-2 text-sm">Save</button>
                        </div>
